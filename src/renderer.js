// renderer.js — BBox-fitted projection + Carto tile stitching
const { createCanvas } = require("canvas");
const { geoNaturalEarth1, geoMercator, geoEquirectangular, geoPath, geoGraticule } = require("d3-geo");
const sharp = require("sharp");
const fetch = require("node-fetch");

// ── Tile providers (raster, no labels) ───────────────────────
...
function getD3Style(k) { return D3_STYLES[k] || D3_STYLES["dark"]; }

// ── Projection — ALWAYS fits bbox ────────────────────────────
function buildProjection(bbox, width, height) {
  const isFullWorld = !bbox ||
    ((bbox.maxLon - bbox.minLon) > 340 && (bbox.maxLat - bbox.minLat) > 160);
  ...
  // Use Mercator for zoomed views (matches MapLibre/web tiles)
...
function calcZoom(bbox) {
  ...
  if(r>300)return 2; if(r>150)return 3; if(r>80)return 4;
  if(r>40)return 5; if(r>20)return 6; if(r>10)return 7;
  if(r>5)return 8; if(r>2)return 9; return 10;
}

async function stitchTiles(bbox, width, height, styleKey) {
  const template = TILE_PROVIDERS[styleKey];
...
// ── Main render — tries tiles first, falls back to D3 ─────────
async function renderBaseMap(world, bbox, options={}) {
  ...
  try {
    const tileBuf = await stitchTiles(bbox, width, height, style);
    if (tileBuf) {
      console.log(`[Renderer] Tiles OK: ${style}`);
      return tileBuf;
    }
  } catch(e) { console.log("[Renderer] Tile fetch failed, using D3:", e.message); }

  console.log(`[Renderer] D3 fallback: ${style}`);
